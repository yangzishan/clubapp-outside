...
export default {
  ...
  _initHeaderImg(imgUrl) {
    ...
    return `https://img.xlxt.net/${imgUrl}`;
  },
  ...
  emarkingPathFrom(url) {
    let urlLink = window.location.href.split("#")[0].toLocaleLowerCase();
    let urlLinkTwo = window.location.href.split("#/")[1];
    if(url){
      let newUrl = url.split("#")[0].toLocaleLowerCase();
      if(url.includes("http")){
        if(newUrl == urlLink){
          if(url.includes("?")){
            localStorage.setItem("activityFrom", ((url).split("#/")[1]).split("?")[0]);
          }else{
            localStorage.setItem("activityFrom", ((url).split("#/")[1]));
          }
          localStorage.setItem("startActivityFrom", urlLinkTwo);
          if(url.indexOf("?") != -1){
            router.push({
              path: url.split("#/")[1] + "&activityFrom=" + urlLinkTwo
            });
          } else {
            router.push({
              path: url.split("#/")[1] + "?activityFrom=" + urlLinkTwo
        ...
        url.indexOf("salesList") != -1 ||
        url.indexOf("courseList") != -1 ||
        url.indexOf("articleList") != -1 ||
        url.indexOf("activitySign") != -1 ||
        url.indexOf("activityModule") != -1 ||
        url.indexOf("wordsPractice") != -1
      ) {
        router.push({
          path: url
        });
  ...
  // 处理头像地址
  _initHeaderImg(imgUrl) {
    // 兼容头像数据返回  null 或者 返回0 的情况
    if (!imgUrl || imgUrl === '0') {
      return "https://img.xlxt.net/photoManage/2020/05/11/15891837490008010.png"
    }
    if (imgUrl.includes("//img.")) {
      return imgUrl;
    }
